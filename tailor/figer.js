 ...
 */
; (function () {
  //两点之间距离 (主要用来算pinch的比例的, 两点之间的距离比值求pinch的scale)
  function getLen(v) {
  ...
  AlloyFinger.prototype = {
    start: function (evt) {
      ...
      if (len > 1) {// 一根手指以上
        ...
        var v = { x: evt.touches[1].pageX - this.x1, y: evt.touches[1].pageY - this.y1 };// 2个手指坐标的差值
        preV.x = v.x;// 差值赋值给PreV对象
        preV.y = v.y;
        this.pinchStartLen = getLen(preV); // start()方法中2点之间的距离
        this.multipointStart.dispatch(evt, this.element);  // (用户的multipointStart回调就在此调用的)
      }
      this._preventTap = false;
      this.longTapTimeout = setTimeout(function () {
        this.longTap.dispatch(evt, this.element); // (用户的longTap回调就在此调用的)
    ...
    move: function (evt) {
      ...
      if (len > 1) { // 2根手指以上(处理捏pinch和旋转rotate手势)
        ...
        var v = { x: evt.touches[1].pageX - currentX, y: evt.touches[1].pageY - currentY };// 第二根手指和第一根手指坐标的差值
        ...
        if (this.x2 !== null && this.sx2 !== null) {// 一根手指第一次必然为空,因为初始化赋值为null, 下面将会用x2, y2保存上一次的结果
          evt.deltaX = (currentX - this.x2 + sCurrentX - this.sx2) / 2; // 拖动过程中或者手指移动过程中的差值(当前坐标与上一次的坐标)
          evt.deltaY = (currentY - this.y2 + sCurrentY - this.sy2) / 2;
        } else {
          evt.deltaX = 0; // 第一次嘛, 手指刚移动,哪来的差值啊,所以为0呗
          evt.deltaY = 0;
        }
        this.twoFingerPressMove.dispatch(evt, this.element);// deltaXY挂载到evt对象上,抛给用户(用户的pressMove回调就在此调用的)
      ...
      if (evt.touches.length < 2) { // 手指数量小于2就触发 (用户的多点结束multipointEnd回调函数)
        this.multipointEnd.dispatch(evt, this.element); // 触发(用户的touchEnd回调函数)
      ...
      if ((this.x2 && Math.abs(this.x1 - this.x2) > 30) ||
        ...
        this.tapTimeout = setTimeout(function () {
          ...
          if (self.isDoubleTap) {// 如果满足双击的话
            self.doubleTap.dispatch(evt, self.element);// 触发(用户的doubleTap事件的回调函数)
